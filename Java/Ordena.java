  ...
  public static void main(String[] args) throws IOException {
    ...
    while(!numParticipantes.equals(0)){
        ...
        while(listaFinal.size() > 1){
            ...
            //Se PAR roda pra direita, se IMPAR roda pra esquerda
            if( direita ){
                System.out.printf("%d passos ==> DIR\n", contaPassos);
                if (contaPassos == 0 && !primeiraRodada)
                    numeroPosicaoAtual--;
                else
                    numeroPosicaoAtual += contaPassos;

                if (numeroPosicaoAtual == listaFinal.size() )
                    numeroPosicaoAtual = 0;
            }
            else{
                System.out.printf("%d passos ==> ESQ\n", contaPassos);
                if (contaPassos == 0 && !primeiraRodada)
                    numeroPosicaoAtual++;
                else
                    numeroPosicaoAtual -= contaPassos;
